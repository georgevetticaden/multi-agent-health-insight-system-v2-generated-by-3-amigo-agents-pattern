...
class LLMTestJudge:
    ...
    async def analyze_complexity_mismatch(
        ...
        try:
            ...
            return PromptRecommendation(
                ...
                complexity_indicators=["Review complexity classification criteria"],
                medical_context="Automated analysis failed",
    ...
    async def judge_specialist_similarity(
        ...
        prompt_file = Path(__file__).parent.parent.parent / "agents" / "cmo" / "judge_prompts" / "specialist" / "judge_similarity.txt"
        prompt_template = prompt_file.read_text()
    ...
    async def analyze_tool_usage_issues(
        ...
        prompt_file = Path(__file__).parent.parent.parent / "agents" / "cmo" / "judge_prompts" / "tool_usage" / "analyze_issues.txt"
        prompt_template = prompt_file.read_text()
    ...
    async def analyze_response_structure_issues(
        ...
        # Load prompt from external file
        prompt_file = Path(__file__).parent.parent.parent / "agents" / "cmo" / "judge_prompts" / "response_structure" / "analyze_issues.txt"
        prompt_template = prompt_file.read_text()
    ...
    async def generate_prompt_improvement_report(
        ...
        if prompt_files_to_review:
            ...
            report.append("1. `backend/services/agents/cmo/prompts/1_gather_data_assess_complexity.txt` - Complexity classification & analysis quality")
            report.append("2. `backend/services/agents/cmo/prompts/3_assign_specialist_tasks.txt` - Specialist selection & task delegation")
            report.append("3. `backend/services/agents/cmo/prompts/2_define_analytical_approach.txt` - Analytical approach definition")
